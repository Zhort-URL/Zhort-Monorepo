<div>
    ...
    <div id="sidebar" class="relative z-50 lg:hidden" role="dialog" aria-modal="true">
        ...
        >
            ...
            >
                ...
                >
                    <!-- Close sidenav -->
                    <button onclick="toggleSidebar()" id="closeSidebarButton" type="button" class="-m-2.5 p-2.5">
                ...
                <!-- Sidebar -->
                <div
                    class="flex grow flex-col gap-y-5 overflow-y-auto dark:bg-zhort-dark px-6 pb-2 border-r border-zhort-darkish"
                ...
                <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    <li>
                        <ul role="list" class="-mx-2 space-y-1">
                            <li>
                                <!-- Current: "bg-indigo-700 text-white", Default: "text-indigo-200 hover:text-white hover:bg-indigo-700" -->
                                <a href="{{ route('dashboard') }}" class="transition fade-in-out duration-200 hover:bg-zhort-darker hover:text-zhort-primary text-zhort-gray group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
                            ...
                            @if (Auth::user()->admin)
                                <li>
                                    <a href="{{ route('admin.users') }}" class="transition fade-in-out duration-200 hover:bg-zhort-darker hover:text-zhort-primary text-zhort-gray group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
                                        <svg class="h-6 w-6 shrink-0 group-hover:text-zhort-primary text-zhort-gray" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                    ...
                    <li class="-mx-6 mt-auto">
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-white border-t border-zhort-darkish bg-zhort-darker/50 hover:bg-zhort-darker transition transition-colors duration-200">
                            <img class="h-8 w-8 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="">
                            <span class="sr-only">Your profile</span>
                            <div class="flex flex-col">
                                <span aria-hidden="true">{{ \Illuminate\Support\Facades\Auth::user()->name }}</span>
                                <span class="text-xs text-zhort-grayer -mt-1">
                                    @if (Auth::user()->admin)
                                        Administrator
                                    @else
                                        User
    ...
    <!-- TOPBAR MOBILE -->
    <div class="sticky top-0 z-40 flex items-center gap-x-6 bg-zhort-dark py-4 px-4 shadow-sm sm:px-6 lg:hidden border-b border-zhort-darkish">
        <!-- OPEN SIDENAV -->
        <button onclick="toggleSidebar()" id="openSidebarButton" type="button" class="-m-2.5 p-2.5 lg:hidden hover:cursor-pointer" >
            <span class="sr-only">Open sidebar</span>
            <svg class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
            </svg>
        </button>
        <!-- TODO: Make text reflect what page you're on -->
        <div href="{{ route('dashboard') }}" class="flex-1 text-sm font-semibold leading-6 text-white">Dashboard</div>
        <a href="{{ route('profile.edit') }}">
            <span href="{{ route('profile.edit') }}" class="sr-only">Your profile</span>
            <img class="h-8 w-8 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="">
        </a>
    </div>
